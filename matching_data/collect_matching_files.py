...
def get_files_in_study_period(directory, years, months, days):
...
def get_file_at_msg_timestamp(directory, timestamp, msg_res=15):
    ...
    closest_files = []
    for f in glob.glob(f"{directory}/{dt[:4]}/{dt[4:6]}/{dt[6:8]}/{dt[:8]}*_E{dt[9:11]}*.nc"):

        start_msg = int(dt[-4:])
        end_msg = start_msg + msg_res if (int(dt[-2:])+msg_res) < 60 else start_msg + (40+msg_res)
        start_data = int(f.split('_')[-3][1:])
        end_data = int(f.split('_')[-2][1:])
...
def get_closest_msg_files(directory, timestamp, msg_res=15):
    ...
    # for f in glob.glob(f"{directory}/{dt[:4]}/{dt[4:6]}/{dt[:8]}*_E{dt[9:11]}*.nc"):
